...
关联文档:[架构设计](./ARCHITECTURE.md) · [技术问题记录](./TECH-NOTES.md) · [**交接简报**](./HANDOFF.md)

> 🔄 **接手 / 新会话请先读 [HANDOFF.md](./HANDOFF.md)** —— 那里固化了三次源码调研中
> **还没落地成代码**的部分(尤其滑条贴图的完整规格),以及全部已知的刻意近似。

---

## 一句话现状

**M0 完成,M1 已经能"只丢一个 `.osr` 进来就播"** —— 自动按谱面 MD5 从镜像站取回 `.osz`,解包挑出正确难度,连音乐一起装好(真实浏览器实测 **6 秒**),然后跑完整链路:堆叠 → 判定(circle + 滑条部件 + 转盘) → 记分(ScoreV1 与 ScoreV2 两套) → combo 累积。渲染侧的 combo 配色与滑条体已按 ppy/osu 源码对齐。**722 个单测全绿,0 个 `todo` 占位。**
...
| M4 | stable 皮肤系统(.osk,预设 + 用户上传) | 🟡 **解析 + 分层兜底已完成**(`.osk` / `skin.ini` / `@2x` / 默认皮肤 75 个贴图,见 [D17](./TECH-NOTES.md) [D18](./TECH-NOTES.md));贴图**绘制**路径未做 |
